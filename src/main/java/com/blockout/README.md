...
BlockOut uses a hierarchical structure, consisting of a root Window, which contains one or more Panes
and Views.  Views are a type of Pane which can contain other Panes (and Views).

Position coordinates in BlockOut are relative, where a Pane's x,y position is relative to the top-left
of the parent View.  There are various means of declaring the size and position of a Pane.
...
If a Window requires no special code, you can directly instantiate com.blockout.views.Window.
Otherwise, create a Java class that extends com.blockout.views.Window.  Override Window methods
to add additional behavior, such as click handling.

If your Window subclass implements com.blockout.controls.Button.Handler, it will automatically receive
button events via the onButtonClicked()


### Layout Files
While you can programmatically create a BlockOut layout, the easiest method is to use a BlockOut layout
files, which is an XML file that describes the layout of the Window.

Layout File Definition
----------------------
BlockOut Layout Files are XML files.  There are two root elements supported: <window> and <layout>.

A Window must reference an XML file that uses a <window> root element, while files with a <layout> root
element are for embedding via inclusion with the <layout> child element.
...
The 'xmlns:xsi' and 'xsi:noNamespaceSchemaLocation' attributes are supported for either root element,
and are optional, but their use allows you to point to the xsd file for editor validation purposes.
...
Attributes:
    ...
    * Integer: 1632768
    * String Name: aqua, black, blue, cyan, fuchsia, green, ivory, lime, magenta, orange,
        orangered, purple, red, white, yellow, gray, darkgray, dimgray, lightgray, slategray
...
Generic attributes:
    ...
    * style="{style}"           If a additional styles have been registered for a Pane type, this will switch the style
...
{width} and {height} attributes may be either a number (e.g, "100" or "100px") or a percentage (e.g, "10%").
Percentage is treated as a percent of the parent's width or height, respectively.

A negative {width} or {height} is treated as 100% of the parent minus the absolute value.  E.g, if the parent is
100x100 pixels, and the pane has a specified size of "-25 -30%" it will have a size of 75x70.

Use either a 'size' attribute **or** a 'width' and/or 'height' attribute, but do not use both.

Positioning:
    * align="{align}"           (default=TopLeft) Alignment
    * pos="{x} {y}"             Position pair defining the x and y position of the Pane within it's parent
    * x="{x}"                   x position only
    * y="{y}"                   y position only

{x} and {y} attributes follow the same rules as {width} and {height}.

Use either a 'pos' attribute **or** an 'x' and/or 'y' attribute, but do not use both.
...
In the case of Bottom alignment modes, the 'y' attribute is reversed - a positive 'y' will move the Pane up.  Likewise,
in the case of Right alignment modes, the 'x' attribute is reversed and moves the Pane left instead of right.
...
Attributes:
    * padding="{pad}"           The usable interior of the View is reduced by the 2x the size in both directions, and
                                elements are offset by the padding in both directions
...
#### List View <list>
A scrolling list, using identical rows.
...
Attributes:
    * source="{path}"           (Required) path to the texture png to use; supports Resource Location path format
...
Attributes:
    * source="{path}"           Path to the XML layout file to embed.  May use Resource Location path
                                (e.g, "modid:path/under/assets/to/layout.xml")
