...
public class CommandListColonies implements IMCCommand
{
    ...
    private int executeCommand(final CommandContext<CommandSource> context, final int startpage)
    {
        ...
        for (final IColony colony : coloniesPage)
        {
            context.getSource()
                .sendFeedback(
                    new StringTextComponent(String.format(ID_AND_NAME_TEXT, colony.getID(), colony.getName())).setStyle(new Style()
                        .setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, String.format(COMMAND_COLONY_INFO, colony.getID())))),
                    true);
            final BlockPos center = colony.getCenter();

            final ITextComponent teleport = new StringTextComponent(
                COORDINATES_TEXT + String.format(COORDINATES_XYZ, center.getX(), center.getY(), center.getZ()));
            teleport.setStyle(new Style().setBold(true)
                .setColor(TextFormatting.GOLD)
                .setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, TELEPORT_COMMAND + colony.getID())));

            context.getSource().sendFeedback(teleport, true);
        }

        final ITextComponent prevButton = new StringTextComponent(PREV_PAGE).setStyle(new Style().setBold(true)
            .setColor(TextFormatting.GOLD)
            .setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, LIST_COMMAND_SUGGESTED + prevPage)));

        final ITextComponent nextButton = new StringTextComponent(NEXT_PAGE).setStyle(new Style().setBold(true)
            .setColor(TextFormatting.GOLD)
            .setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, LIST_COMMAND_SUGGESTED + nextPage)));
        ...
        context.getSource()
            .sendFeedback(
                beginLine.appendSibling(prevButton)
                    .appendSibling(new StringTextComponent(PAGE_LINE_DIVIDER))
