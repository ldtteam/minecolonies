...
public class CheckForAutoDeletesCommand extends AbstractSingleCommand
{
    ...
    public void execute(@NotNull final MinecraftServer server, @NotNull final ICommandSender sender, @NotNull final String... args) throws CommandException
    {
        ...
        for (int index = 0; colonies.size() - 1 >= index; index++)
        {
            ...
            if (colony.canBeAutoDeleted() && Configurations.autoDeleteColoniesInHours != 0 && colony.getLastContactInHours() >= Configurations.autoDeleteColoniesInHours)
            {
        ...
        if (args.length != 0)
        {
            if ("true".equalsIgnoreCase(args[0]))
            {
                ...
                {
                    server.addScheduledTask(() -> ColonyManager.deleteColony(col.getID(), Configurations.autoDestroyColonyBlocks));
                }
            }
        }
        else
