...
public class CommandCitizenList implements IMCColonyOfficerCommand
{
    ...
    private int displayListFor(final CommandContext<CommandSource> context, int page)
    {
        ...
        final ITextComponent headerLine = LanguageHandler
            .buildChatComponent("com.minecolonies.command.citizenlist.pagetop", page, pageCount);
    ...
    @NotNull
    private List<ICitizenData> getCitizensOnPage(final List<ICitizenData> citizens,
        final int citizenCount,
        final int pageStartIndex,
        final int pageStopIndex)
    {
    ...
    private void drawCitizens(@NotNull final CommandContext<CommandSource> context, final List<ICitizenData> citizensPage)
    {
        for (final ICitizenData citizen : citizensPage)
        {
            context.getSource()
                .sendFeedback(
                    LanguageHandler.buildChatComponent("com.minecolonies.command.citizeninfo.desc", citizen.getId(), citizen.getName())
            ...
            citizen.getCitizenEntity().ifPresent(entityCitizen -> {
                ...
                context.getSource()
                    .sendFeedback(LanguageHandler
                        .buildChatComponent("com.minecolonies.command.citizeninfo.pos", position.getX(), position.getY(), position.getZ()),
                        true);
            });
        }
    }
     ...
     */
    private static void drawPageSwitcher(@NotNull final CommandContext<CommandSource> context,
        final int page,
        final int count,
        final int halfPage,
        final int colonyId)
    {
        ...
        final ITextComponent prevButton = LanguageHandler.buildChatComponent("com.minecolonies.command.citizenlist.prev")
            .setStyle(new Style().setBold(true)
                .setColor(TextFormatting.GOLD)
                .setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, String.format(LIST_COMMAND_SUGGESTED, colonyId, prevPage))));
        final ITextComponent nextButton = LanguageHandler.buildChatComponent("com.minecolonies.command.citizenlist.next")
            .setStyle(new Style().setBold(true)
                .setColor(TextFormatting.GOLD)
                .setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, String.format(LIST_COMMAND_SUGGESTED, colonyId, nextPage))));
        ...
        context.getSource()
            .sendFeedback(
                beginLine.appendSibling(prevButton)
