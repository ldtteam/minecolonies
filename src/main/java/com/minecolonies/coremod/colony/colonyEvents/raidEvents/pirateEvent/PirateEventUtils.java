...
public final class PirateEventUtils
{
    ...
    private static void loadSpawners(final World world, final BlockPos targetSpawnPoint, final String shipSize, final IColony colony, final int eventID)
    {
        switch (shipSize)
        {
            ...
            case MEDIUM_PIRATE_SHIP:
                setupSpawner(targetSpawnPoint.up(3).north(10), world, PIRATE_CHIEF, colony, eventID);
                setupSpawner(targetSpawnPoint.up(1), world, PIRATE, colony, eventID);
                setupSpawner(targetSpawnPoint.up(5).south(6), world, PIRATE_ARCHER, colony, eventID);
                break;
            case BIG_PIRATE_SHIP:
                ...
                setupSpawner(targetSpawnPoint.up(3).north(10), world, PIRATE_CHIEF, colony, eventID);
                setupSpawner(targetSpawnPoint.up(6).north(12), world, PIRATE_CHIEF, colony, eventID);

                setupSpawner(targetSpawnPoint.up(9).north(13), world, PIRATE_ARCHER, colony, eventID);
                setupSpawner(targetSpawnPoint.up(22).south(), world, PIRATE_ARCHER, colony, eventID);
                setupSpawner(targetSpawnPoint.up(6).south(11), world, PIRATE_ARCHER, colony, eventID);
                break;
            default:
     ...
     */
    private static void setupSpawner(final BlockPos location, final World world, final ResourceLocation mob, final IColony colony, final int eventID)
    {
        ...
        spawner.getSpawnerBaseLogic().spawnData.getNbt().setInteger(TAG_EVENT_ID, eventID);
        spawner.getSpawnerBaseLogic().spawnData.getNbt().setInteger(TAG_COLONY_ID, colony.getID());
    ...
    {
        if (spawnPoint.equals(colony.getCenter()) || spawnPoint.getY() > Configurations.gameplay.maxYForBarbarians)
        {
            return false;
        }
     ...
     */
    public static boolean canPlaceShipAt(final BlockPos pos, final Structure ship, final IBlockAccess world)
    {
