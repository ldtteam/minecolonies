...
public class ExpeditionLog
{
    ...
    public void serializeNBT(@NotNull final CompoundNBT compound)
    {
        ...
        final ListNBT equipment = new ListNBT();
        ...
        {
            final CompoundNBT mob = new CompoundNBT();
            mob.putString(TAG_TYPE, entry.getKey().getRegistryName().toString());
            mob.putInt(TAG_COUNT, entry.getValue());
            mobs.add(mob);
        }
        compound.put(TAG_MOBS, mobs);

        final ListNBT loot = new ListNBT();
    ...
    public void deserializeNBT(@NotNull final CompoundNBT compound)
    {
        ...
        final ListNBT equipment = compound.getList(TAG_EQUIPMENT, Constants.NBT.TAG_COMPOUND);
        ...
        this.mobs.clear();
        final ListNBT mobs = compound.getList(TAG_MOBS, Constants.NBT.TAG_COMPOUND);
        for (int i = 0; i < mobs.size(); ++i)
        {
            final CompoundNBT mob = mobs.getCompound(i);
            final ResourceLocation type = new ResourceLocation(mob.getString(TAG_TYPE));
            final EntityType<?> entityType = ForgeRegistries.ENTITIES.getValue(type);
            if (entityType != null)
            {
        ...
        this.loot.clear();
        final ListNBT loot = compound.getList(TAG_LOOT, Constants.NBT.TAG_COMPOUND);
    ...
    public void serialize(@NotNull final PacketBuffer buf)
    {
        ...
        for (final Map.Entry<EntityType<?>, Integer> entry : this.mobs.entrySet())
        {
            buf.writeRegistryIdUnsafe(ForgeRegistries.ENTITIES, entry.getKey());
    ...
    public void deserialize(@NotNull final PacketBuffer buf)
    {
        ...
        for (int size = buf.readVarInt(); size > 0; --size)
        {
            final EntityType<?> entityType = buf.readRegistryIdUnsafe(ForgeRegistries.ENTITIES);
