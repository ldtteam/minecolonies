...
public class BuildingCook extends AbstractBuildingSmelterCrafter
{
    ...
    {
        super(c, l);
        keepX.put((stack) -> this.hasModule(ItemListModule.class) && !this.getModuleMatching(ItemListModule.class, m -> ((ItemListModule) m).getId().equals(FOOD_EXCLUSION_LIST)).map(m -> m.isItemInList(new ItemStorage(stack))).orElse(false), new Tuple<>(STACKSIZE, true));
    ...
    public int buildingRequiresCertainAmountOfItem(final ItemStack stack, final List<ItemStorage> localAlreadyKept, final boolean inventory)
    {
        ...
        if (ISFOOD.test(stack) && this.hasModule(ItemListModule.class) && !this.getModuleMatching(ItemListModule.class, m -> ((ItemListModule) m).getId().equals(FOOD_EXCLUSION_LIST))
