...
public class BuildingCook extends AbstractBuildingSmelterCrafter
{
    ...
    {
        super(c, l);
        keepX.put((stack) -> this.hasModule(ItemListModule.class) && !this.getModuleMatching(ItemListModule.class, m -> m.getId().equals(FOOD_EXCLUSION_LIST)).isItemInList(new ItemStorage(stack)), new Tuple<>(STACKSIZE, true));
