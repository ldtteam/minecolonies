    ...
    public static class Factory implements IFactory<FactoryVoidInput, StandardRequestSystemBuildingDataStore>
    {
        ...
        public StandardRequestSystemBuildingDataStore deserialize(@NotNull final IFactoryController controller, @NotNull final CompoundNBT nbt) throws Throwable
        {
            ...
            final Map<TypeToken<?>, Collection<IToken<?>>> openRequestsByRequestableType = NBTUtils
                                                                                             .streamCompound(nbt.getList(TAG_OPEN_REQUESTS_BY_TYPE, Constants.NBT.TAG_COMPOUND)).map(nbtTagCompound -> {
                  final TypeToken<?> key = controller.deserialize(nbtTagCompound.getCompound(TAG_TOKEN));
                  final Collection<IToken<?>> values = NBTUtils.streamCompound(nbtTagCompound.getList(TAG_LIST, Constants.NBT.TAG_COMPOUND))
            ...
            final Map<Integer, Collection<IToken<?>>> openRequestsByCitizen = NBTUtils
                                                                                             .streamCompound(nbt.getList(TAG_OPEN_REQUESTS_BY_CITIZEN, Constants.NBT.TAG_COMPOUND)).map(nbtTagCompound -> {
                  final Integer key = controller.deserialize(nbtTagCompound.getCompound(TAG_TOKEN));
                  final Collection<IToken<?>> values = NBTUtils.streamCompound(nbtTagCompound.getList(TAG_LIST, Constants.NBT.TAG_COMPOUND))
                                                         .map(elementCompound -> (IToken<?>) controller.deserialize(elementCompound))
                                                         .collect(Collectors.toList());
            ...
            final Map<Integer, Collection<IToken<?>>> completedRequestsByCitizen = NBTUtils
                                                                                .streamCompound(nbt.getList(TAG_COMPLETED_REQUESTS_BY_CITIZEN, Constants.NBT.TAG_COMPOUND)).map(nbtTagCompound -> {
                  final Integer key = controller.deserialize(nbtTagCompound.getCompound(TAG_TOKEN));
                  final Collection<IToken<?>> values = NBTUtils.streamCompound(nbtTagCompound.getList(TAG_LIST, Constants.NBT.TAG_COMPOUND))
                                                         .map(elementCompound -> (IToken<?>) controller.deserialize(elementCompound))
                                                         .collect(Collectors.toList());
            ...
            final Map<IToken<?>, Integer> citizenByOpenRequest = NBTUtils
                                                                                     .streamCompound(nbt.getList(TAG_CITIZEN_BY_OPEN_REQUEST, Constants.NBT.TAG_COMPOUND)).map(nbtTagCompound -> {
                  final IToken<?> key = controller.deserialize(nbtTagCompound.getCompound(TAG_TOKEN));
                  final Integer value = controller.deserialize(nbtTagCompound.getCompound(TAG_VALUE));
