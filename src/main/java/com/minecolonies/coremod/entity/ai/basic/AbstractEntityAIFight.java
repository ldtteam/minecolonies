        ...
        for (final List<GuardGear> itemList : itemsNeeded)
        {
            for (final GuardGear item : itemList)
            {
                ...
                if (items.isEmpty())
                {
                    ...
                    if ((item.getType().isArmor() && ItemStackUtils.isEmpty(worker.getInventoryCitizen().getArmorInSlot(item.getType()))) || ItemStackUtils.isEmpty(worker.getItemBySlot(item.getType())))
                    {
                        // create request
