...
public class EntityAIWorkCook extends AbstractEntityAIUsesFurnace<JobCook, BuildingCook>
{
    ...
    protected boolean reachedMaxToKeep()
    {
        ...
        return InventoryUtils.getCountFromBuildingWithLimit(building, ItemStackUtils.CAN_EAT.and(stack -> stack.getFoodProperties(null).getNutrition() >= building.getBuildingLevel() - 1), stack -> stack.getMaxStackSize() * 6) > buildingLimit;
    }
        ...
        if (InventoryUtils.isItemHandlerFull(handler))
        {
            if (!citizenToServe.isEmpty())
            {
                ...
                if (foodSlot != -1)
                {
                    ...
                    if (stack.isEdible())
                    {
                        final AbstractEntityCitizen citizen = citizenToServe.get(0);
                        citizen.getCitizenData().increaseSaturation(stack.getFoodProperties(citizen).getNutrition() / 2.0);
                    }
    ...
    protected IRequestable getSmeltAbleClass()
    {
        ...
        // remove if nutrition < building.getBuildingLevel() - 1
        blockedItems.removeIf(item -> !item.hasSufficientNutrition(building.getBuildingLevel() - 1, null));
