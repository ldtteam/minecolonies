...
public class EntityAIWorkComposter extends AbstractEntityAIInteract<JobComposter, BuildingComposter>
{
    ...
    private IAIState getMaterials()
    {
        ...
        if (slot >= 0)
        {
            worker.setLastHandItem(Hand.MAIN_HAND, worker.getInventoryCitizen().getStackInSlot(slot));
    ...
    private IAIState fillBarrels()
    {
        ...
        if (worker.getLastHandItem(Hand.MAIN_HAND) == ItemStack.EMPTY)
        {
            final int slot = InventoryUtils.findFirstSlotInItemHandlerWith(
        ...
        if (world.getBlockEntity(currentTarget) instanceof TileEntityBarrel)
        {
            ...
            barrel.addItem(worker.getLastHandItem(Hand.MAIN_HAND));
    ...
    private void complain()
    {
        if (ticksToComplain <= 0)
        {
            ...
            {
                player.sendMessage(new TranslationTextComponent(COM_MINECOLONIES_COREMOD_ENTITY_COMPOSTER_EMPTYLIST), player.createPlayerUUID());
            }
        }
        else
        {
