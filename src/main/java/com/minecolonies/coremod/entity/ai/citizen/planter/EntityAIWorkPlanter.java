...
public class EntityAIWorkPlanter extends AbstractEntityAICrafting<JobPlanter, BuildingPlantation>
{
    ...
    private IAIState farm()
    {
        ...
        if (!holdEfficientTool(world.getBlockState(workPos.above()), workPos.above()))
        {
            return START_WORKING;
        }
        ...
        for (final ItemEntity item : world.getEntitiesOfClass(ItemEntity.class, new AABB(worker.blockPosition()).expandTowards(4.0F, 1.0F, 4.0F).expandTowards(-4.0F, -1.0F, -4.0F)))
        {
            if (item != null)
            {
