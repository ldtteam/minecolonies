    ...
    extends AbstractStructureHandler
{
     ...
     */
    public BuildingStructureHandler(final World world,
    ...
    public void prePlacementLogic(final BlockPos worldPos, final BlockState blockState)
    {
        ...
        if (MathHelper.floor(structureAI.getWorker().getPosX()) == worldPos.getX()
            ...
            && MathHelper.floor(structureAI.getWorker().getPosZ()) == worldPos.getZ() && structureAI.getWorker().getNavigator().noPath())
        {
    ...
    {
        return itemStack == null || itemStack.isEmpty() || itemStack.getItem().isIn(ItemTags.LEAVES)
