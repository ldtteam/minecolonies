...
public class EntityAIVisitor extends Goal
{
    ...
    private boolean reduceTime()
    {
        ...
        if (citizen.getCitizenData().getSaturation() <= 0)
        {
            ...
            if (tavern != null)
            {
                ((TavernCoreBuildingStatsModule) tavern.getModule(TavernCoreBuildingStatsModule.class)).removeCitizen(citizen.getCivilianID());
            }
            return true;
        }
        return false;
    ...
    private boolean doFight()
    {
        ...
        if (citizen.isWorkerAtSiteWithMove(target.getPosition(), 2) && citizen.canEntityBeSeen(target))
        {
            citizen.swingArm(Hand.MAIN_HAND);
            target.attackEntityFrom(new NamedDamageSource(citizen.getName().getFormattedText(), citizen), 10.0f);
        }
    ...
    private VisitorState decide()
    {
        ...
        {
            final BlockPos pos = ((TavernCoreBuildingStatsModule) tavern.getModule(TavernCoreBuildingStatsModule.class)).getFreeSitPosition();
            if (pos != null)
            {
