...
public abstract class AbstractPathJob implements Callable<Path>
{
    ...
    {
        this(world, start, startRestriction, endRestriction, range, Vector3i.ZERO, hardRestriction, result, entity, restrictionType);
    }
    ...
    public void synchToClient(final LivingEntity mob)
    {
        for (final Iterator<Map.Entry<PlayerEntity, UUID>> iter = trackingMap.entrySet().iterator(); iter.hasNext(); )
        {
            ...
            {
                Network.getNetwork().sendToPlayer(new SyncPathMessage(debugNodesVisited, debugNodesNotVisited, debugNodesPath), (ServerPlayerEntity) entry.getKey());
            }
        }
    }
    ...
    public static BlockPos prepareStart(@NotNull final LivingEntity entity)
    {
        ...
        while (!bs.getMaterial().blocksMotion() && !down.getMaterial().blocksMotion() && !down.getBlock().isLadder(down, world, pos.below(), entity) && down.getFluidState()
          .isEmpty())
        {
     ...
     */
    protected boolean isPassable(@NotNull final BlockState block, final BlockPos pos, final Node parent, final boolean head)
    {
     ...
     * @return true if so.
     */
    private boolean canLeaveBlock(final BlockPos pos, final Node parent, final boolean head)
    {
        BlockPos parentPos = parent == null ? start : parent.pos;
        if (head)
        {
