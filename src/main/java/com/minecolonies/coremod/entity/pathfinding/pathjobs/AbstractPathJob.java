...
public abstract class AbstractPathJob implements Callable<Path>
{
    ...
    public static BlockPos prepareStart(@NotNull final LivingEntity entity)
    {
        ...
        while (!bs.getMaterial().blocksMotion() && !down.getMaterial().blocksMotion() && !down.getBlock().isLadder(down, entity.getCommandSenderWorld(), pos.below(), entity))
        {
    ...
    private void walkCurrentNode(@NotNull final Node currentNode)
    {
        ...
        if (isPassable(currentNode.pos.below(), false))
        {
            walk(currentNode, BLOCKPOS_DOWN);
        }
    ...
    {
        BlockPos pos = parent.pos.relative(dPos);
        ...
        if (pos.getY() != newY)
        {
            ...
            if (!parent.isCornerNode() && newY - pos.getY() > 0 && (parent.parent == null || !parent.parent.pos.equals(parent.pos.relative(new BlockPos(0, newY - pos.getY(), 0)))))
            {
                dPos = new BlockPos(0, newY - pos.getY(), 0);
                pos = parent.pos.relative(dPos);
                corner = true;
            }
            ...
            {
                dPos = dPos.relative(0, newY - pos.getY(), 0);
    ...
    private int checkDrop(@Nullable final Node parent, @NotNull final BlockPos pos, final boolean isSwimming)
    {
        ...
        //  Nothing to stand on
        if (!canDrop || isSwimming || ((parent.pos.getX() != pos.getX() || parent.pos.getZ() != pos.getZ()) && isPassable(parent.pos.below(), false)
    ...
    private int handleTargetNotPassable(@Nullable final Node parent, @NotNull final BlockPos pos, @NotNull final BlockState target)
    {
        ...
        if (!isPassable(pos.above(2), false))
        {
        ...
        if (!isPassable(parent.pos.above(2), false))
        {
            final VoxelShape bb1 = world.getBlockState(pos).getCollisionShape(world, pos);
