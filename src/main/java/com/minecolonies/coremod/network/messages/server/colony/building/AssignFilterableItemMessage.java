...
import com.minecolonies.coremod.colony.buildings.modules.itemlist.ItemListModule;
...
public class AssignFilterableItemMessage extends AbstractBuildingServerMessage<AbstractBuildingWorker>
{
    ...
    {
        // Needs to be seperate messages, for each module, or have some kind of string -> class mapping or alternatively could have an actual ID in each module
        if (building.hasModule(ItemListModule.class))
        {
            if (assign)
            {
                building.getModuleMatching(ItemListModule.class.hashCode(), m -> ((ItemListModule) m).getId().equals(id)).ifPresent(m -> m.addItem(item));
            }
            else
            {
