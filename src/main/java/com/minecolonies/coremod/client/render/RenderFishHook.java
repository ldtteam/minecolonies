    ...
    public void render(Entity entity, float ratio, float partialTicks, @NotNull MatrixStack matrixStack, IRenderTypeBuffer iRenderTypeBuffer, int light)
    {
        ...
        if (citizen != null)
        {
            ...
            if ((this.renderManager.options == null || this.renderManager.options.getPointOfView().func_243192_a()))
            {
                ...
                d4 = MathHelper.lerp((double) partialTicks, citizen.prevPosX, citizen.getPosX()) + Vector3d.x;
                d5 = MathHelper.lerp((double) partialTicks, citizen.prevPosY, citizen.getPosY()) + Vector3d.y;
                d6 = MathHelper.lerp((double) partialTicks, citizen.prevPosZ, citizen.getPosZ()) + Vector3d.z;
                f3 = citizen.getEyeHeight();
            }
            else
            {
                d4 = MathHelper.lerp((double) partialTicks, citizen.prevPosX, citizen.getPosX()) - d1 * d2 - d0 * 0.8D;
                d5 = citizen.prevPosY + (double) citizen.getEyeHeight() + (citizen.getPosY() - citizen.prevPosY) * (double) partialTicks - 0.45D;
                d6 = MathHelper.lerp((double) partialTicks, citizen.prevPosZ, citizen.getPosZ()) - d0 * d2 + d1 * 0.8D;
