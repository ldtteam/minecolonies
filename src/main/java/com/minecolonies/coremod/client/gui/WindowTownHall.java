...
public class WindowTownHall extends AbstractWindowBuilding<ITownHallView>
{
    ...
    private void createAndSetStatistics()
    {
        ...
        final int citizensCap = (int)(Math.min(MineColonies.getConfig().getCommon().maxCitizenPerColony.get(), citizenCapBoost != null ? 25 + citizenCapBoost.getEffect() : 25));
        ...
        {
            hoverText.add(LanguageHandler.format("com.minecolonies.coremod.gui.townHall.population.totalcitizens.houselimited", this.building.getColony().getName()));
            findPaneOfTypeByID(TOTAL_CITIZENS_LABEL, Label.class).setColor(ORANGE);
        }
        else
        {
            if(citizensCap < MineColonies.getConfig().getCommon().maxCitizenPerColony.get())
            {
                hoverText.add(LanguageHandler.format("com.minecolonies.coremod.gui.townHall.population.totalcitizens.researchlimited", this.building.getColony().getName()));
            }
            else
            {
                hoverText.add(LanguageHandler.format( "com.minecolonies.coremod.gui.townHall.population.totalcitizens.configlimited", this.building.getColony().getName()));
            }
            findPaneOfTypeByID(TOTAL_CITIZENS_LABEL, Label.class).setLabelText
                                                                    (LanguageHandler.format(COM_MINECOLONIES_COREMOD_GUI_TOWNHALL_POPULATION_TOTALCITIZENS_COUNT,
                                                                      citizensSize,
