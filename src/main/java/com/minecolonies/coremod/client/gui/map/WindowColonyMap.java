    ...
    private void updateColonyInfoImage(final ColonyListMessage.ColonyInfo colonyInfo)
    {
        ...
        if (colonyPane == null)
        {
            ...
            PaneBuilders.tooltipBuilder().hoverPane(colonyPane)
              .append(new TextComponent("Owner:" + colonyInfo.getOwner()))
              .appendNL(new TextComponent("Coordinates: " + colonyInfo.getCenter().getX() + "X, " + colonyInfo.getCenter().getZ() + "Z"))
              .appendNL(new TextComponent("Citizens: " + colonyInfo.getCitizencount()))
              .build();
        }
    ...
    private void updateBuildingView(final IBuildingView buildingView)
    {
        ...
        if (uiBuilding == null)
        {
            ...
            tooltip.hoverPane(uiBuilding)
              .append(window.getScreen().getTooltipFromItem(item).get(0)).append(new TextComponent(" : " + buildingView.getBuildingLevel()))
              .appendNL(new TextComponent("Coordinates: " + buildingView.getID().getX() + "X, " + buildingView.getID().getZ() + "Z"))
              .appendNL(new TextComponent("Citizens: " + buildingView.getAllAssignedCitizens().size()));
            ...
            for (int id : buildingView.getAllAssignedCitizens())
            {
                ...
                if (dataView != null)
                {
                    tooltip.appendNL(new TextComponent(dataView.getName()));
                }
            }
            tooltip.build();
        ...
        for (final ICitizenDataView data : colony.getCitizens().values())
        {
            ...
            if (citizen != null)
            {
                ...
                if (!data.getJob().isEmpty())
                {
                    ...
                    builder.newLine().append(new TranslatableComponent("com.minecolonies.coremod.gui.citizen.job.label", LanguageHandler.format(data.getJob())));
                }
