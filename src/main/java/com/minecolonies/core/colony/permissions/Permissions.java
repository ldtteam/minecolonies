...
public class Permissions implements IPermissions
{
    ...
    public boolean hasPermission(@NotNull final Player player, @NotNull final Action action)
    {
        ...
        else if (action.allowsOperatorBypass() && player.hasPermissions(IMinecoloniesAPI.getInstance().getConfig().getServer().permissionEventMinBypassPermLevel.get()))
        {
     ...
     *
     * @param buf        {@link RegistryFriendlyByteBuf} to write to.
