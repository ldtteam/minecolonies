...
public abstract class AbstractEntityAIBasic<J extends AbstractJob> extends AbstractAISkeleton<J>
{
    ...
    private boolean checkForTool(@NotNull String tool)
    {
       final boolean needsTool = !InventoryFunctions
        ...
        final boolean toolLevel = verifyToolLevel(tool);
        final int hutLevel = worker.getWorkBuilding().getBuildingLevel();

        if (!needsTool && toolLevel)
        {
            return false;
        }
        delay += DELAY_RECHECK;
    ...
    private boolean checkForPickaxe(final int minlevel)
    {
        ...
        final int hutLevel = worker.getWorkBuilding().getBuildingLevel();
        final boolean toolLevel = verifyToolLevel(Utils.PICKAXE);
    ...
    private int getMostEfficientTool(@NotNull final Block target)
    {
        ...
        for (int i = 0; i < inventory.getSizeInventory(); i++)
        {
            ...
            if (level >= required && level < bestLevel)
            {
                if(item!=null && tool != "")
                {
                    final boolean toolVerify = verifyToolLevel2(item, level);
                    if (toolVerify)
                    {
                        bestSlot = i;
                        bestLevel = level;
    ...
    private boolean verifyToolLevel(final String tool)
    {
        @NotNull InventoryCitizen inventory = worker.getInventoryCitizen();
        ...
        for (int i = 0; i < inventory.getSizeInventory(); i++)
        {
            ...
            if(item!=null)
            {
                ...
                if (isTool && verifyToolLevel2(item, level))
                    return true;
            }
        }
        return false;
    ...
    private boolean verifyToolLevel2(final ItemStack item, int level)
    {
        final int hutLevel = worker.getWorkBuilding().getBuildingLevel();
