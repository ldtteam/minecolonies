...
public class EntityAIWorkFarmer extends AbstractEntityAIInteract<JobFarmer>
{
    ...
    */
    ...
    public void make_land()
    {
        ...
        {
            if(world.getBlockState(new BlockPos(farmAbleLand.get(0).getX(), farmAbleLand.get(0).getY() - 1, farmAbleLand.get(0).getZ())).getBlock() != Blocks.farmland)
            {
                delay = 20;
                world.setBlockState(new BlockPos(farmAbleLand.get(0).getX(), farmAbleLand.get(0).getY() - 1, farmAbleLand.get(0).getZ()), Blocks.farmland.getDefaultState());
                currentFarmLand = new BlockPos(farmAbleLand.get(0).getX(), farmAbleLand.get(0).getY() - 1, farmAbleLand.get(0).getZ());
            }

            if(plowedLand.isEmpty() || !plowedLand.contains(new BlockPos(farmAbleLand.get(0).getX(), farmAbleLand.get(0).getY(), farmAbleLand.get(0).getZ())))
            {
                plowedLand.add(new BlockPos(farmAbleLand.get(0).getX(), farmAbleLand.get(0).getY(), farmAbleLand.get(0).getZ()));
        ...
        if(!plowedLand.isEmpty())
        {
            if(world.getBlockState(plowedLand.get(0)).getBlock() == Blocks.farmland)
            {
                if(world.isAirBlock(plowedLand.get(0)))
                {
                    ...
                    delay = 20;
                    currentFarmLand = new BlockPos(plowedLand.get(0).getX(), plowedLand.get(0).getY() - 1, plowedLand.get(0).getZ());
                ...
                if(crops.size() == 0 || !crops.contains(new BlockPos(plowedLand.get(0).getX(), plowedLand.get(0).getY(), plowedLand.get(0).getZ())))
                {
                    crops.add(new BlockPos(plowedLand.get(0).getX(), plowedLand.get(0).getY(), plowedLand.get(0).getZ()));
                    plowedLand.remove(0);
                }
            }
            else
            {
                if(farmAbleLand.isEmpty() || !farmAbleLand.contains(new BlockPos(plowedLand.get(0).getX(), plowedLand.get(0).getY(), plowedLand.get(0).getZ())))
                {
                    farmAbleLand.add(new BlockPos(plowedLand.get(0).getX(), plowedLand.get(0).getY(), plowedLand.get(0).getZ()));
                    plowedLand.remove(0);
                }
            }
        }
    ...
    private void harvesting()
    {
        if(!crops.isEmpty())
        {
            ...
            if(BlockPosUtil.isWorkerAtSiteWithMove(worker, crops.get(0)))
            {
                ...
                if(block == Blocks.melon_block || block == Blocks.pumpkin || world.getBlockState(crops.get(0)).getValue(BlockCrops.AGE) == 0x7)
                {
    ...
    private boolean isInHut(Item item)
    {
        ...
        for(int i = 0; i < size; i++)
        {
            ...
            {
                Item content = stack.getItem();
                if(content.equals(item) || content.getToolClasses(stack).contains(needItem) || stack.getUnlocalizedName().contains(needItem))
                {
                    ItemStack returnStack = InventoryUtils.setStack(getInventory(), stack);
