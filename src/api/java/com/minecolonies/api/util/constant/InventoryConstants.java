package com.minecolonies.api.util.constant;

/**
 * Some constants needed for the whole mod.
 */
public final class InventoryConstants
{
    /**
     * Amount of columns in the player inventory.
     */
    public static final int INVENTORY_COLUMNS = 9;

    /**
     * Initial x-offset of the inventory slot.
     */
    public static final int PLAYER_INVENTORY_INITIAL_X_OFFSET = 8;

    /**
     * Initial y-offset of the inventory slot.
     */
    public static final int PLAYER_INVENTORY_INITIAL_Y_OFFSET = 31;

    /**
     * Each offset of the inventory slots.
     */
    public static final int PLAYER_INVENTORY_OFFSET_EACH = 18;

    /**
     * Initial y-offset of the inventory slots in the hotbar.
     */
    public static final int PLAYER_INVENTORY_HOTBAR_OFFSET = 89;

    /**
     * Amount of rows in the player inventory.
     */
    public static final int INVENTORY_ROWS = 3;

    /**
     * The size of the the inventory hotbar.
     */
    public static final int INVENTORY_BAR_SIZE = 8;

    /**
     * The size of a normal inventory.
     */
    public static final int MAX_INVENTORY_INDEX = 28;

    /**
     * X-Offset of the inventory slot in the GUI of the scarecrow.
     */
    public static final int X_OFFSET = 80;

    /**
     * Y-Offset of the inventory slot in the GUI of the scarecrow.
     */
    public static final int Y_OFFSET = 34;

    /**
     * Initial y-offset of the inventory slot.
     */
    public static final int PLAYER_INVENTORY_INITIAL_Y_OFFSET_CRAFTING = 84;

    /**
     * Each offset of the inventory slots.
     */
    public static final int INVENTORY_OFFSET_EACH = 18;

    /**
     * Initial y-offset of the inventory slots in the hotbar.
     */
    public static final int PLAYER_INVENTORY_HOTBAR_OFFSET_CRAFTING = 142;

    /**
     * The x position of the crafting result slot position.
     */
    public static final int X_CRAFT_RESULT = 124;

    /**
     * The y position of the crafting result slot position.
     */
    public static final int Y_CRAFT_RESULT = 35;

    /**
     * The x offset of the crafting slot position.
     */
    public static final int X_OFFSET_CRAFTING = 30;

    /**
     * The y offset of the crafting slot position.
     */
    public static final int Y_OFFSET_CRAFTING = 17;

    /**
     * Amount of slots in the crafting window.
     */
    public static final int CRAFTING_SLOTS = 5;

    /**
     * Amount of slots in the crafting window.
     */
    public static final int FURNACE_SLOTS = 2;

    /**
     * Amount of slots in the crafting window.
     */
    public static final int ADDITIONAL_SLOTS = 5;

    /**
     * Start slot of the player hotbar.
     */
    public static final int HOTBAR_START = 32;

    /**
     * Total amount of slots in the GUI.
     */
    public static final int TOTAL_SLOTS = 41;

    /**
     * Total amount of slots in the furnace GUI.
     */
    public static final int TOTAL_SLOTS_FURNACE = 38;

    /**
     * Private constructor to hide implicit public one.
     */
    private InventoryConstants()
    {
        /**
         * Intentionally left empty.
         */
    }
}
