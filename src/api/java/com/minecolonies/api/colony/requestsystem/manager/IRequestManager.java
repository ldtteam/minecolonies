...
public interface IRequestManager extends INBTSerializable<NBTTagCompound>, ITickable
{
    ...
    /**
     * Method used to start a simulation from this point.
     * @return A {@link IRequestManager} that functions as a simulation.
     */
    @NotNull
    IRequestManager startSimulation();

    /**
     * Method used to mark a simulation as complete and commit its changes to this manager.
     * @param simulatingRequestManager The simulation who's changes need to be committed.
     */
    @NotNull
    void commitSimulation(@NotNull final IRequestManager simulatingRequestManager);

    /**
     * Method used to find out if this manager is simulating a resolving operation.
     * @return True when simulating, false when not
     */
    default boolean isSimulating()
    {
        return false;
    }
}
