...
@SuppressWarnings({"LawOfDemeter", "ClassWithoutLogger", "NewMethodNamingConvention", "PublicMethodWithoutLogging", "ConditionalExpression"})
    ...
    private static List<ItemStack> calculateSecondaryOutputs(@NotNull final IRecipe<?> recipe)
    {
        if (recipe instanceof ICraftingRecipe)
        {
            ...
            return ((ICraftingRecipe) recipe).getRemainingItems(inv).stream()
                    ...
                    .filter(stack -> stack.getItem() != buildTool)  // this is filtered out of the inputs too
                    .collect(Collectors.toList());
        }
