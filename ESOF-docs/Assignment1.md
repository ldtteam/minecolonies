...
Relativamente ao trabalho, geralmente todos os contribuidores trabalham naquilo que querem e no que mais apreciam. Quando existe uma grande mudança no jogo base, (por exemplo, mudanças de versões do minecraft), a fluidez do trabalho pode mudar. No entanto, na maior parte dos casos, existe alguém responsável por expor alguns detalhes mais urgentes de solucionar e, a partir daí, os contribuidores escolhem o que fazem.

#### Código ####

Relativamente ao código, é exigido aos contribuidores que sigam [regras](https://github.com/Minecolonies/minecolonies/wiki) bastante específicas de *code style*. Embora sejam regras que possam parecer excessivas, num projeto *Open Source* é conveniente que todos tenham a mesma formatação de código de modo a tornar-lo legível e percetível a qualquer interessado. 

O código é estruturado em branches e são também usados testes unitários.

Existem 3 revisores de código que maioritariamente verificam o código submitido e após estar pronto, utilizam o SonarQube que irá comentar todos os possíveis problemas que encontrou. Finalmente quando todos os revisores de código verificarem o trabalho do SonarQube (e feito as devidas alterações) o trabalho do programador pode ser implementado.
...
Para quem está ligado ao **MineColonies**, tanto programador como utilizador, existe a [comunidade](http://minecolonies.com/) onde há a discussão de assuntos por fóruns e por onde pode ser feito o *report* de erros/*bugs* ou sugestões. Na nossa opinião, a comunidade é um fator chave para o sucesso da equipa devido ao contacto amigável e bastante responsivo entre programadores-utilizadores.
Ainda na comunidade, qualquer utilizador pode descarregar o jogo com facilidade e ainda existe a divulgação de [tutorias](http://minecolonies.com/wiki), o que torna a comunidade ainda mais integrante. 

Passando para os novos contribuidores, existe uma série de instruções ([TODO list](http://www.minecolonies.com/forum/m/9402108/viewthread/8293794-main-todo-list)) e regras sobre a organização de código ([code style](https://github.com/Minecolonies/minecolonies/wiki)) para quem quer começar, que devem ser respeitadas. Na nossa opinião é um dos pontos mais positivos, visto que torna uma leitura e interpretação mais legível e uniforme do mesmo, evitando *refactorings*.

O facto de a discussão de assuntos entre programadores ser feito a nível do [Slack](https://minecolonies.slack.com/) permite um feedback bastante rápido, o que facilita a resolução de alguns problemas ou questões diretas. Também têm objetivos a curto, médio e longo prazo o que demonstra que é um projecto que terá continuidade e espaço para evoluir. 
...
Na nossa opinião, embora seja difícil adequar um modelo teórico a um projeto *Open Source*, o método de processo que a equipa do **MineColonies** utiliza é o mais adequado. Isto verifica-se pois torna a maneira como o projeto evolui bastante mais dinâmica do que qualquer outro modelo. 

Comparando com o modelo de cascata, apesar deste poder ser utilizado, não seria tão adequado uma vez que modificações de *Minecraft* é um mercado bastante competitivo. Cada modificação tem de seguir um modelo propício à inovação rápida e com bastantes novidades. Se tal não acontecer a modificação rapidamente perderá a adesão que tem pois envelhecerá rapidamente.

Relativamente aos modelos de protótipo de software e reutilização de software, consideramos que, neste tipo de projetos, não são propriamente possíveis de implementar com sucesso. Isto deve-se ao facto de serem essencialmente dirigidos a projetos de grandes empresas que querem, ou validar e tirar conclusões com um protótipo, ou poupar dinheiro em código. Ora, visto que é um projeto *Open Source* que é feito por quem quiser e sem pagamentos, poupar dinheiro em código está fora de questão. Lançar um protótipo, pelo menos nesta fase já avançada, nao faria muito sentido uma vez que acarretaria um volume de trabalho desnecessariamente excessivo à execução do mesmo.
...
* [Mário Fernandes](https://github.com/MarioFernandes73) (up201201705@fe.up.pt) - 30%

* [Manuel Curral](https://github.com/Camolas)  (up201202445@fe.up.pt) - 10%
